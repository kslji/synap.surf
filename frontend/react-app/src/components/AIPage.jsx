...
export default function AIPage() {
  ...
  if (!wallet) {
    return (
      <div className="main-content fade-in" style={{ display: 'flex', flexDirection: 'column', flex: 1, justifyContent: 'center', alignItems: 'center', background: 'radial-gradient(circle at 50% 0%, rgba(255, 159, 67, 0.1) 0%, var(--bg) 60%)', height: '100vh' }}>
        <div style={{ textAlign: 'center', maxWidth: 400, background: 'var(--card)', padding: 32, borderRadius: 16, border: '1px solid var(--border)' }}>
          <h2 style={{ fontSize: 24, marginBottom: 16, color: 'var(--t1)' }}>Wallet Required</h2>
          <p style={{ color: 'var(--t3)', lineHeight: 1.6 }}>Please connect your wallet to use the AI chat. The AI needs access to your specific trade history to provide accurate responses.</p>
          <button 
            onClick={() => window.dispatchEvent(new Event('trigger_wallet_connect'))}
            style={{ marginTop: 24, padding: '12px 24px', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, margin: '24px auto 0' }}
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 12V8H6a2 2 0 0 1-2-2c0-1.1.9-2 2-2h12v4" /><path d="M4 6v12c0 1.1.9 2 2 2h14v-4" /><path d="M18 12a2 2 0 0 0-2 2c0 1.1.9 2 2 2h4v-4h-4z" /></svg>
            Connect Wallet
          </button>
        </div>
      </div>
