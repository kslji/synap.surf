...
export default function TradeHistory({ trades }) {
  const closed = trades.filter(t => t.event === 'TRADE_CLOSE');
  return (
    <section className="table-section">
      ...
      <div className="data-table">
        <div className="dt-head">
          <span>Asset</span><span>Type</span><span>Entry</span><span>Exit</span><span>P&amp;L</span><span>Size</span>
        </div>
        {!closed.length
          ...
          : closed.map((t, i) => {
            ...
            return (
              <div key={i} className="dt-row">
                ...
                <div className="dt-col">${(t.exit_price || 0).toFixed(2)}</div>
                <div className={`dt-col dt-pnl ${isPos ? 'pos' : 'neg'}`}>
                  {isPos ? '+' : '-'}${Math.abs(t.pnl_usd || 0).toFixed(2)}
                </div>
