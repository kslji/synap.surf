# file: strategies/stoch_slow_reversal.py
import pandas as pd
...
class StochSlowReversalStrategy:
    ...
    def run(self, df: pd.DataFrame) -> Dict[str, Any]:
        min_len = self.stoch_length + self.smooth_d + 1
        if len(df) < min_len:
        ...
        ll = df["low"].rolling(self.stoch_length).min()
        hh = df["high"].rolling(self.stoch_length).max()
        df["%K"] = ((df["close"] - ll) / (hh - ll) * 100).rolling(self.smooth_k).mean()
        df["%D"] = df["%K"].rolling(self.smooth_d).mean()
        ...
        for i in range(1, len(df)):
            ...
            long_sig  = (prev["%K"] <= prev["%D"]) and (row["%K"] > row["%D"]) and (row["%K"] < self.oversold)
            short_sig = (prev["%K"] >= prev["%D"]) and (row["%K"] < row["%D"]) and (row["%K"] > self.overbought)
