# file: strategies/pivot_reversal_stop.py
import pandas as pd
import numpy as np
...
class PivotReversalStopStrategy:
    ...
    def run(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        pending_long = np.nan; pending_short = np.nan
        trades: list = []
        ...
        for i in range(start, len(df)):
            ...
            if position != 0:
                ...
                if hit == "sl":
                    ...
                    pending_long = pending_short = np.nan; equity.append(eq); continue
                if hit == "tp":
            ...
            if not pd.isna(row["pl"]): pending_long  = row["pl"]  + self.tick_offset
            if not pd.isna(row["ph"]): pending_short = row["ph"]  - self.tick_offset
 
            long_trig  = (not np.isnan(pending_long))  and row["high"] >= pending_long
            short_trig = (not np.isnan(pending_short)) and row["low"]  <= pending_short
 
            if long_trig and position <= 0:
                if position == -1:
                    eq, position, entry_price, entry_time = close_trade(trades, eq, position, entry_price, entry_time, pending_long, dt, self.position_size_pct, "opposite_breakout")
                position = 1; entry_price = max(row["open"], pending_long); entry_time = dt; pending_long = np.nan
            elif short_trig and position >= 0:
                if position == 1:
                    eq, position, entry_price, entry_time = close_trade(trades, eq, position, entry_price, entry_time, pending_short, dt, self.position_size_pct, "opposite_breakout")
                position = -1; entry_price = min(row["open"], pending_short); entry_time = dt; pending_short = np.nan
