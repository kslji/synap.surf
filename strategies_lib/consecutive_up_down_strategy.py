# file: strategies_lib/consecutive_up_down_strategy.py
import pandas as pd
...
class ConsecutiveStreakStrategy:
    ...
    def run(self, df: pd.DataFrame) -> Dict[str, Any]:
        if len(df) < max(self.consecutive_up, self.consecutive_down) + 1:
            return {"error": "Not enough data for selected streak length"}
        ...
        for i in range(1, len(df)):
            ...
            long_sig  = up_streak   >= self.consecutive_up
            short_sig = down_streak >= self.consecutive_down

            if long_sig and position <= 0:
                if position == -1:                   # BUG 2
                    eq, position, entry_price, entry_time = close_trade(
                        trades, eq, position, entry_price, entry_time,
                        row["close"], dt, self.position_size_pct, "opposite_signal")
                position    = 1
