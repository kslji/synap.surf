...
class SmaCrossoverStrategy:
    ...
    def run(self, df: pd.DataFrame) -> Dict[str, Any]:
        if len(df) < self.slow_length + 1:
            return {"error": "Not enough data for SMA calculation"}
        ...
        df["fast_ma"] = df["close"].rolling(self.fast_length).mean()
        df["slow_ma"] = df["close"].rolling(self.slow_length).mean()
        ...
        for i in range(1, len(df)):
            ...
            long_sig  = (prev["fast_ma"] <= prev["slow_ma"]) and (row["fast_ma"] > row["slow_ma"])
            short_sig = (prev["fast_ma"] >= prev["slow_ma"]) and (row["fast_ma"] < row["slow_ma"])
