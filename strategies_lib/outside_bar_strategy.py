# file: strategies_lib/outside_bar_strategy.py
import pandas as pd
...
class OutsideBarStrategy:
    """Outside Bar reversal — all 5 bugs fixed."""
    ...
    def run(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for i in range(1, len(df)):
            ...
            is_outside = (row["high"] > prev["high"]) and (row["low"] < prev["low"])
            if not is_outside:
                equity.append(eq); continue
