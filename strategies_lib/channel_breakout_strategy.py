# file: strategies_lib/channel_breakout_strategy.py
import pandas as pd
...
class ChannelBreakoutStrategy:
    """Donchian-style Channel Breakout — all 5 bugs fixed."""

    def __init__(self, initial_capital=1000.0, position_size_pct=1.0,
                 channel_length=5, tick_offset=0.1,
    ...
    def run(self, df: pd.DataFrame) -> Dict[str, Any]:
        if len(df) < self.channel_length + 2:
            return {"error": f"Not enough data (need ≥ {self.channel_length + 2} bars)"}
        ...
        df["up_bound"]   = df["high"].rolling(self.channel_length).max().shift(1)
        df["down_bound"] = df["low"].rolling(self.channel_length).min().shift(1)
        ...
        for i in range(self.channel_length + 1, len(df)):
            ...
            if position != 0:
                ...
                if hit == "sl":
                    eq, position, entry_price, entry_time = close_trade(
                        ...
                        sl, dt, self.position_size_pct, "sl_hit")
            ...
            # ── Breakout signals ──────────────────────────────────────────────
            buy_stop  = row["up_bound"]   + self.tick_offset if pd.notna(row["up_bound"])   else float("inf")
            sell_stop = row["down_bound"] - self.tick_offset if pd.notna(row["down_bound"]) else float("-inf")

            long_break  = row["high"] >= buy_stop
            short_break = row["low"]  <= sell_stop

            if long_break and position <= 0:
                if position == -1:                   # BUG 2 — unconditional close
                    eq, position, entry_price, entry_time = close_trade(
                        trades, eq, position, entry_price, entry_time,
                        max(row["open"], buy_stop), dt,
                        self.position_size_pct, "opposite_breakout")
                position    = 1
                entry_price = max(row["open"], buy_stop)
                entry_time  = dt

            elif short_break and position >= 0:
                if position == 1:
                    eq, position, entry_price, entry_time = close_trade(
                        trades, eq, position, entry_price, entry_time,
                        min(row["open"], sell_stop), dt,
                        self.position_size_pct, "opposite_breakout")
                position    = -1
                entry_price = min(row["open"], sell_stop)
                entry_time  = dt
